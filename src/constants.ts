import {
    FrameColor,
    FrameType,
    TextColor,
    Base64PNG,
} from './types';

export const DisplayWidth = 256;
export const DisplayHeight = 192;

export const TextColorWhite = 'White' as TextColor;
export const TextColorRed = 'Red' as TextColor;
export const TextColorBlue = 'Blue' as TextColor;
export const TextColorGreen = 'Green' as TextColor;
export const TextColorYellow = 'Yellow' as TextColor;
export const DefaultTextColor = TextColorWhite;

export const FrameColorRed = 'Red' as FrameColor;
export const FrameColorBlue = 'Blue' as FrameColor;
export const FrameColorGreen = 'Green' as FrameColor;
export const DefaultFrameColor = FrameColorGreen;

export const FrameType1 = 1 as FrameType;
export const FrameType2 = 2 as FrameType;
export const FrameType3 = 3 as FrameType;
export const FrameType4 = 4 as FrameType;
export const FrameType5 = 5 as FrameType;
export const DefaultFrameType = FrameType1;

interface CornerPNGSet {
    TopRight: Base64PNG,
    BottomRight: Base64PNG,
}

export const FrameData = new Map<FrameType, CornerPNGSet>([
    [
        FrameType1,
        {
            TopRight: '',
            BottomRight: '',
        },
    ] as [FrameType, CornerPNGSet],
]);
